...
export default function MenuCard({item, isFavorited}: {item: MenuItemType, isFavorited: boolean}) {
    
    return (
        <div className="w-full h-96 col-span-1 shadow relative group">
            <div className="w-full h-2/3 relative overflow-hidden">
                <img className="w-full h-full object-cover group-hover:scale-110 transition-all duration-300" src={item.image} alt={item.name}/>
                <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-500 ">
                        ...
                        :
                        <button onClick={()=> unfavoriteAction(item.id)} className="hover:scale-110 transition-all duration-200">
                            <Heart className="size-10 text-secondary"/>
                        </button>
                        }
                </div>
            </div>
